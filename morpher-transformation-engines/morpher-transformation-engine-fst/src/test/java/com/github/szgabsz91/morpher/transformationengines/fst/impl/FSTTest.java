...
public class FSTTest {
    ...
    public void testTransformWithEmptyTransducer() {
        ...
        Optional<TransformationEngineResponse> response = fst.transform(input);
    ...
    public void testTransformWithNonEmptyTransducer() {
        FST fst = new FST();
        Word input = Word.of("a");
        TransformationEngineResponse expected = TransformationEngineResponse.singleton(Word.of("b"));
    ...
    public void testLearnAndBuildPhasesAfterEachOther() {
        ...
        fst.learn(batch1);
        batch1.forEach(wordPair -> assertThat(fst.transform(wordPair.getLeftWord())).hasValue(TransformationEngineResponse.singleton(wordPair.getRightWord())));
        fst.learn(extraWordPair);
        batch1.forEach(wordPair -> assertThat(fst.transform(wordPair.getLeftWord())).hasValue(TransformationEngineResponse.singleton(wordPair.getRightWord())));
        assertThat(fst.transform(extraWordPair.getLeftWord())).hasValue(TransformationEngineResponse.singleton(extraWordPair.getRightWord()));
    }
    ...
    public void testLearnWithAmbiguousWordPairs() {
        ...
        fst.learn(wordPairs);
        assertThat(fst.transform(Word.of("a"))).hasValue(TransformationEngineResponse.singleton(Word.of("b")));
    }
